...
# data1=pd.DataFrame(columns=['area_code','area_name','level','parent_code'])
# conn=pymysql.connect(host='127.0.0.1',user='root',password='popeye12.', db='work', charset='utf8')
# data=data.append(pd.read_sql('select * from work.area_code_2021 limit 100',conn))
# print(data)


async def dbdaochu(loop):
    # 生成一个空的DataFrame结构
    dd=pd.DataFrame(data=[(None,None,None,None),],columns=['area_code','area_name','level','parent_code'])
    sqlstr='select * from work.area_code_2021 limit 10'
